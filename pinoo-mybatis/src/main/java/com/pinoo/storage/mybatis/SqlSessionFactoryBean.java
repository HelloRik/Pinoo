...
import com.pinoo.storage.mybatis.builder.XMLConfigBuilder;
import com.pinoo.storage.mybatis.cache.ICache;
 ...
 * @version $Id$
 * 
 *          重写IBATIS的SqlSessionFactoryBean,支持读写分离，目前只支持3.2.2版本
...
public class SqlSessionFactoryBean implements FactoryBean<SqlSessionFactory>, InitializingBean,
    ...
    protected SqlSessionFactory buildSqlSessionFactory() throws IOException {
        ...
        ConfigurationExtend configuration;

        XMLConfigBuilder xmlConfigBuilder = null;
        if (this.configLocation != null) {
            xmlConfigBuilder = new XMLConfigBuilder(this.configLocation.getInputStream(), null,
                    this.configurationProperties);
            configuration = (ConfigurationExtend) xmlConfigBuilder.getConfiguration();
        } else {
            if (this.logger.isDebugEnabled()) {
                this.logger.debug("Property 'configLocation' not specified, using default MyBatis Configuration");
            }
            configuration = new ConfigurationExtend();
        ...
        configuration.setCache(this.cache);
