...
public class ReflectionUtil {
    ...
    public static Class<?> getMethodReturnGenricType(Method method, int index) {
        ...
        if (index >= params.length || index < 0) {
            logger.warn("Index: " + index + ", Size of " + method.getName() + "'s Parameterized Type: " + params.length);
            return Object.class;
        }
        if (!(params[index] instanceof Class)) {
            logger.warn(method.getName() + " not set the actual class on superclass generic parameter");
            return Object.class;
        }
        return (Class) params[index];
    }
